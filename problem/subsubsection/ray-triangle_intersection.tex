...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[tdplot_main_coords,thick,scale=2]
    ...
    % triangle
    \draw [blue!50, thick, -{Stealth[width=3mm, length=3mm]}] (P) -- (2,-3,2);
    ...
    % \draw [blue!50, thick, -{Stealth[width=2mm, length=3mm]}] (B) -- (P);
    % \draw [blue!50, thick, -{Stealth[width=3mm, length=3mm]}] (B) -- (3,-3,1);
    % \draw [blue!50, thick, -{Stealth[width=3mm, length=3mm]}] (C) -- (A);
    % \draw [blue!50, thick, -{Stealth[width=2mm, length=3mm]}] (C) -- (P);
    % \draw [blue!50, thick, -{Stealth[width=2mm, length=3mm]}] (C) -- (1,-3,2);
    \node [above] at (2,-4,3) {$A$};
    \draw plot [mark=*, mark size=1] coordinates{(2,-4,3) }; 
    \node [left] at (3,-4,1) {$B$};
    \draw plot [mark=*, mark size=1] coordinates{(3,-4,1) }; 
    \node [below] at (1,-4,2) {$C$};
    \draw plot [mark=*, mark size=1] coordinates{(1,-4,2) };
    \node [left] at (2.5,-4,2) {$\vv{a}$};
    \node [above] at (2,-3.5,2) {$\vv{n}$};
    \node [above] at (2,-3.5,3) {$\vv{n_a}$};
    \node [right] at (2,-4,2.5) {$\vv{AP}$};
    % \node [below] at (1,-4,2) {$\vv{b}$};
    % \node [below] at (1,-4,2) {$\vv{c}$};
    
    % ray
    \node [below left] at (P) {$P$};
    \draw plot [mark=*, mark size=1] coordinates{(2,-4,2) };
  \end{tikzpicture}
  \caption{Viser krydsproduktet $\vv{n_a}$ af en af trekantens sider $\vv{a}$ og $\vv{AP}$.}
  \label{fig:perspektiv_projektion}
\end{figure}
