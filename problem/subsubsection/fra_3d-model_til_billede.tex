...
\label{fig:kanin}
    \centering
    \includegraphics[widt0h=5cm]{kanin}
...
\end{tikzpicture}
  \caption{Viser hvordan et punkt på billedplanet som en kombination af tre vektorer $\protect\vv{r}$, $\protect\vv{u}$ og $\protect\vv{f}$, der repræsentere hhv. højre, op og frem retninger for kameraet.}
  \label{fig:kamera_billede}
\end{figure}

Et punkt $B$ på billedplanet (Se figur \ref{fig:kamera_billede}) kan representere en pixel og kan beskrives som en linear kombination af en op og en højre orientationsvektor samt en vektor der beskriver afstanden til billedplanet relativt til kammeraet. På figuren er dette vektor $\vv{f}$ scaleret med en faktor $d$.

Et pixel koordinat relativt til kameraets udgangspunkt $C$ kan altså beskrives på følgende måde:
\begin{equation}
  \vv{f} \cdot d + \vv{u} \cdot (y-\frac{h}{2}) + \vv{r} \cdot (x-\frac{w}{2}
\end{equation}
